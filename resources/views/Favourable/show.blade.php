...
<div id="urHere">积云教育管理中心<b>></b><strong>帮助中心</strong> </div>   <div class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
        ...
        <h3><a href="{{url('favourable/shows')}}" class="actionBtn add">添加帮助数据</a>帮助中心{{Session::get('message')}}</h3>
    ...
    <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
     <tr>
      ...
      <tr>
        ...
        <td><a href="{{url('favourable/msg')}}/{{$v->id}}" >{{$v->bt}}</a></td>
        <!-- 即点即改-->
       <td onclick="updateUser({{$v->id}})">
        ...
        <td align="center">
          <a href="{{url('favourable/update')}}/{{$v->id}}">编辑</a> | 
          <a href="{{url('favourable/delete')}}/{{$v->id}}">删除</a>
        </td>
      </tr>
      ...
      @endforeach  
       <tr align="center">
        <td colspan="8"> 
          ...
          <button class="pl" style="width:100px;height:30px;">批量删除</button> 
          <input type="button" value="全选" onclick="qx()" style="width:100px;height:30px;">
          <input type="button" value="反选" onclick="fx()" name="all" style="width:100px;height:30px;">
          <input type="button" value="全不选" onclick="qbx()" style="width:100px;height:30px;">
...
<script>
 ...
 function saveUser(id){
  ...
  $.ajax({
    type:"POST",
    url:"{{url('favourable/saveUsernamed')}}",
    data:{
      id:id,
      fbt:fbt
    },success:function(e){
 ...
 //批量删除  
    $(".pl").click(function(){  
      ...
      for(var i=0;i<length;i++){  
       ...
       location.href="{{url('favourable/del_all')}}?id="+str;  
    }) 
    //全选
    function qx(){
